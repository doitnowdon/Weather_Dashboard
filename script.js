...
$(document).ready(function () {
    ...
    const cityEl = $('h2#city'); // done 
    const dateEl = $('h5#date'); // done 
    const weatherIconEl = $('img#weather-icon'); // done 
    const temperatureEl = $('span#temperature'); // done 
    const humidityEl = $('span#humidity'); // done 
    const windEl = $('span#wind'); // done 
    const uvIndexEl = $('span#uv-index'); // done 
    const cityListEl = $('div.cityList'); // done 

   // Selectors for form elements
   const cityInput = $('#city-input'); // done
    ...
    function searchWeather(queryURL) {
     ...
     function displayLastSearchedCity() {
        if (pastCities[0]) {
            ...
            // if no past searched cities, load Colombo weather data
            let queryURL = buildURLFromInputs("Colombo");
            searchWeather(queryURL);
        }
    }
